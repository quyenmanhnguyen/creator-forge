...
def build_breakdown_system_prompt(template: SceneTemplate, *, n_scenes: int) -> str:
    ...
    return (
        ...
        "6. SCENE-TO-SCENE DIVERSITY — across all "
        f"{n_scenes} scenes, no two IMAGE PROMPTs may open with the "
        "same noun phrase or framing clause, even when the location "
        "and subject are shared. Vary the opening across scenes by "
        "rotating composition (wide establishing / medium / tight "
        "close-up / over-the-shoulder / top-down / mirrored "
        "reflection), camera angle (eye-level / low / high / dutch / "
        "aerial), and detail focus (hero subject / hands / texture / "
        "environmental cue). A reader scanning only the first eight "
        "words of each IMAGE PROMPT must be able to tell the scenes "
        "apart. Likewise vary the FLOW VIDEO PROMPT openings — no "
        "two scenes may begin their motion description with the same "
        "camera move (e.g. don't start every scene with 'A slow dolly "
        "push-in').\n\n"
